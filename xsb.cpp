...
void XSB::replace_outer_floor(ElementsMatrix &m)
{
    ...
    for (auto row=m.zero(); row<m.row_size(); ++row) {
        ...
        //从右到左
        for (auto col=m.col_size()-1; col>=m.zero(); --col) {
            if (test_set(Pos(row, col)))
                break;
        }
    }
    ...
    for (auto col=m.zero(); col<m.col_size(); ++col) {
        ...
        //从下到上
        for (auto row=m.row_size()-1; row>=m.zero(); --row) {
            if (test_set(Pos(row, col)))
                break;
        }
    }
...
static ElementsMatrix remove_row(type_size row, const ElementsMatrix &m)
{
    ...
    for (auto r=m.zero(); r<m.row_size(); ++r) {
        if (r == row) continue;
        auto r_to = r<row ? r : r-1;
        for (auto c=m.zero(); c<m.col_size(); ++c) {
            Pos from(r, c);
            Pos to(r_to, c);
...
static ElementsMatrix remove_col(type_size col, const ElementsMatrix &m)
{
    ...
    for (auto c=m.zero(); c<m.col_size(); ++c) {
        ...
        for (auto r=m.zero(); c<m.row_size(); ++r) {
            Pos from(r, c);
            Pos to(r, c_to);
            em.set(to, m.get(from));
        }
